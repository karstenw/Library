...
# The library depends on the Cocoa ObjC AppKit and Foundation modules (bundled with
# NodeBox), the Python math module, the Python NumPy module (bundled with NodeBox)
# for pixel operations, the Python Core Graphics module (installed on the Mac by
# default) for CMYK conversion, the nodebox.graphics module for drawing operations
# in NodeBox.
# However, all of the classes are loosely connected so it probably isn't that much
# work to make the library work in a different environment.
...
# check the bottom of the documentation webpage
# (http://nodebox.net/code/index.php/Core_Image).
# The main issue: exporting image files leaks memory, which can result in kernel
# panic. I haven't found a solution yet, there's a problem when calling
# CIContext().drawImage.  For now, don't export movies/PDF's/images in batch or
# use an external screen capture tool.
...
class Layers(list):
    ...
    def __getitem__(self, index):
        ...
        When a layer is a canvas, or when a layer's alpha mask is a canvas,
...
class Layer(object):
    ...
    def copy(self):
        ...
        if self.type == LAYER_PATH:
            data = (self.data[0].copy(), Color(self.data[1]),
                                         Color(self.data[2]),
                                         Color(self.data[3]), self.data[4])
        ...
        if self.type == LAYER_NSIMAGEDATA:
            data = self.data
        layer = Layer(self.canvas, self.type, data, self.x, self.y,
                                                    self._w, self._h)
        ...
        layer._distort = [Point(self._distort[i].x, self._distort[i].y)
                          for i in range(4)]
    ...
    def size(self):
        ...
        scaled (whereas the base width and height properties contain the size of
        the layer's data).
        """
        l, t, r, b = self.bounds()
        return (r-l, b-t)
    ...
    def translate(self, x, y):
        ...
        size (between 0.0 and 1.0), or as absolute coordinates in pixels.
        """
    ...
    def scale(self, w=None, h=None):
        ...
        and 1.0), or as absolute coordinates in pixels. 
    ...
    def _set_brightness(self, value=0.0):
        self._brightness = max(-1.0, min(value, 1.0))

    def _set_contrast(self, value=1.0):
        self._contrast = max(0.25, min(value, 4.0))

    def _set_saturation(self, value=1.0):
        self._saturation = max(0.00, min(value, 2.0))
    ...
    def filter(self, name, **kwargs):
        ...
        For example:
        canvas.layers[i].filter(name=canvas.renderer.FILTER_KALEIDOSCOPE,
                                count=10,
                                dx=-100)
        """
    ...
    # a PIL-renderer would be able to perform exactly the same effects
    # as Core Image.
    ...
    def filter_blur(self, amount=10.0):
        self.filter("blur", **{"amount":amount})

    def filter_sharpen(self, amount=2.5):
        self.filter("sharpen", **{"amount":amount})
    
    def filter_zoomblur(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_ZOOMBLUR, **kwargs)

    def filter_motionblur(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_MOTIONBLUR, **kwargs)

    def filter_noisereduction(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_NOISEREDUCTION, **kwargs)

    def filter_bumpdistortion(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_BUMPDISTORTION, **kwargs)

    def filter_linearbump(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_BUMPDISTORTIONLINEAR, **kwargs)

    def filter_holedistortion(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_HOLEDISTORTION, **kwargs)

    def filter_circlesplash(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_CIRCLESPLASHDISTORTION, **kwargs)

    def filter_twirl(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_TWIRLDISTORTION, **kwargs)

    def filter_circularwrap(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_CIRCULARWRAP, **kwargs)

    def filter_kaleidoscope(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_KALEIDOSCOPE, **kwargs)

    def filter_triangletile(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_TRIANGLETILE, **kwargs)

    def filter_parallelogramtile(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_PARALLELOGRAMTILE, **kwargs)

    def filter_perspectivetile(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_PERSPECTIVETILE, **kwargs)

    def filter_starshine(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_STARSHINEGENERATOR, **kwargs)

    def filter_checkerboard(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_CHECKERBOARDGENERATOR, **kwargs)

    def filter_pixelate(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_PIXELATE, **kwargs)

    def filter_crystallize(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_CRYSTALLIZE, **kwargs)

    def filter_dotscreen(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_DOTSCREEN, **kwargs)

    def filter_bloom(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_BLOOM, **kwargs)

    def filter_lighting(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_SPOTLIGHT, **kwargs)

    def filter_shading(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_SHADEDMATERIAL, **kwargs)

    def filter_levels(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_COLORMATRIX, **kwargs)

    def filter_edges(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_EDGES, **kwargs)

    def filter_lineoverlay(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_LINEOVERLAY, **kwargs)

    def filter_pagecurl(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_PAGECURLTRANSITION, **kwargs)

    def filter_histogram(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_HISTOGRAM, **kwargs)

    def filter_average(self, **kwargs):
        self.filter(self.canvas.renderer.FILTER_AVERAGE, **kwargs)
    ...
    def filters_first(self, b=None):
        ...
        Either filters are applied to the layer before scaling, distorting and
        rotating it, or applied last, after having transformed the layer.
        """
...
class CoreImageRenderer(Renderer):
    ...
    def filter(self, name, input=None, parameters={}):
        ...
        filter.setDefaults()
        if input != None:
            filter.setValue_forKey_(input, u"inputImage")
    ...
    def imagesize(self, path):
        s = str(path)
        url = NSURL.fileURLWithPath_( s )
        #if not s.startswith( u"Color" ):
        #    print "INPUTPATH: '%s'" % url
    ...
    def render_filter_motionblur(self, layer, img, options):
        ...
        for filter, params in layer.filters:
            if filter == self.FILTER_PERSPECTIVETILE:
                return img
        (x,y), (w,h) = img.extent() 
        # If the image is opaque at the edges it will will stretch out beyond its
        # bounds.  To solve this, we add a transparent 1-pixel border/gutter around
        # the layer.
        if w < layer.canvas.w*10 and h < layer.canvas.h*10:
            ...
            img = self.filter("CISourceOverCompositing",
                              img, {"inputBackgroundImage": base}) 
        p = {
    ...
    def render_filter_zoomblur(self, layer, img, options):
        ...
        dy = options["dy"]
        p = {
            "inputCenter" : CIVector.vectorWithX_Y_(layer._w * 0.5 + dx,
                                                    layer._h * 0.5 -dy),
    ...
    def render_filter_bumpdistortionlinear(self, layer, img, options):
        ...
        # Left and top edge sometimes get chopped off if we don't
        # accumulate it first.
    ...
    def render_filter_triangletile(self, layer, img, options):
        ...
        img = self.filter("CICrop", img,
                        {"inputRectangle": CIVector.vectorWithX_Y_Z_W_(x, y, w, h)})  
        return img
    ...
    def render_crop(self, layer, img):
        if layer._crop != None:
            ...
            # Also, when the transform queue order changes crop will not
            # behave right (e.g. when first rotating and scaling and then cropping).
            # Also, since we adjust width and height, Canvas.draw() followed by
            # Canvas.export() generates a problem.
            (x,y), (w,h) = img.extent()
            layer._w = w
            layer._h = h
            t = NSAffineTransform.transform()
    ...
    def export(self, canvas, name, typ_=FILE_PNG,
               compression=None, cmyk=False, layers=[]):
...
class CoreImagePixels:
    ...
    def __init__(self, renderer, layer, img):
        ...
        Creates a pixel array for a CIImage object, using NumPy for faster array
        processing.  The pixels values are assumed to be RGB and range between 0 and 255.
    ...
    def min(self, transparency=False, threshold=0):
        ...
        if transparency == False and self.has_alpha():
            # Find the indices of all pixels whose alpha is greater than given
            # threshold.  Take only the pixels at those indices.
