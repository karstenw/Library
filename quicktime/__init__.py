...
# Quicktime is a Mac OS X specific framework available from Mac OS X 1.4 (Tiger)
# and NodeBox 1.9.0.
...
class Media:
    ...
    def __init__(self, data, start=0, stop=None):
        ...
        self.fps = int(1.0
                    / (1.0 * self._qtmovie.currentTime().timeValue
                        / self.timescale))
...
class Movie(Media, object):
    ...
    def __init__(self, data, start=0, stop=None):
        ...
        x = self._qtmovie.attributeForKey_("QTMovieNaturalSizeAttribute")
        w, h = x.sizeValue()
        self.width = w
        self.height = h
    ...
    def frames(self, n=10, start=0, stop=None):
        ...
        for i in range(n-1):
            offset = i / float(n-1)
            offsetTime = int(start + (stop-start) * offset)
            offsetTime = QTMakeTime(offsetTime, self.timescale)
            frame = self._qtmovie.frameImageAtTime_(offsetTime)
            if frame:
                frame = MovieFrame(frame, src=self.path, time=offsetTime)
                images.append(frame)

        if n > 1:
            # Retrieving the very last frame returns None in QuickTime 7.2.0,
            # so we retrieve the one-before-last frame.
            # offsetTime = int(stop - 1)
            offsetTime = int(stop)
        else:
            offsetTime = int(start)
        offsetTime = QTMakeTime(offsetTime, self.timescale)
        frame = self._qtmovie.frameImageAtTime_(offsetTime)
        if frame:
            frame = MovieFrame(frame, src=self.path, time=offsetTime)    
