...
# The NodeBox Web library offers a collection of services to retrieve content from the
# internet.  You can use the library to query Yahoo! for links, images, news and spelling
# suggestions, to read RSS and Atom newsfeeds, to retrieve articles from Wikipedia,
#  to collect quality images from morgueFile, to get color themes from kuler, to browse
#  through HTML documents, to clean up HTML, to validate URL's, to create GIF images
# from math equations using mimeTeX, to get ironic word definitions from Urban Dictionary.

# The NodeBox Web library works with a caching mechanism that stores things you download
# from the web, so they can be retrieved faster the next time. Many of the services also
# work asynchronously. 
# This means you can use the library in an animation that keeps on running while new
# content is downloaded in the background.
