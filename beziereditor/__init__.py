...
# The NodeBox Bezier path editor offers an interactive way to construct paths in
# NodeBox.  Some things are better done with the mouse than with code and creating
# smooth paths just the way you want is one of them. This straightforward editor
# allows you to create paths by adding and moving path points, dragging handles,
# drawing in freehand and organising points on a grid.
#
# Controls:
#
...
# 2) Select a point by clicking on it. You can drag its control handles, move
#    the point and delete it by clicking the button with a cross.
# 3) The last point in the path has an extra pause button. By clicking this you
#    break the path, the next point will start at a new location.
# 4) When dragging a point's handle, hold down "x" to break contuity: this half
#    of the handle will move independently from the other half.
# 5) Hit ESC to deselect the current selected point.
# 6) Hit TAB to activate or deactivate the magnetic grid. Points will snap to
#    it when moved.
# 7) Hit BACKSPACE to delete the current selected point.
# 8) Press "f" to enter freehand mode. You can now draw freehand lines.
#    Press "f" again to exit freehand mode. When you delete the last point in a
#    freehand path, the whole freehand path will be removed.
# 9) Use the LEFT, RIGHT, UP and DOWN keys to move the path around on the canvas.
#
# The path you are creating is automatically stored in an SVG file for which you
# can supply a name. The file is constantly updated with your changes.
# You can then import the file with the SVG library in other NodeBox projects to
# do all sorts of fun and programmatic things with your path. You can also import
# the SVG file in Illustrator or Inkscape to fine-tune it.
...
class MagneticGrid:
    ...
    def snap(self, x, y, treshold=0.4):
        ...
        gx, gy = x, y
        # snap top left
        if x-l < treshold and y-t < treshold: gx, gy = l, t

        # snap top right
        if r-x < treshold and y-t < treshold: gx, gy = r, t

        # snap bottom right
        if r-x < treshold and b-y < treshold: gx, gy = r, b

        # snap bottom left
        if x-l < treshold and b-y < treshold: gx, gy = l, b
        ...
        if (    _ctx._ns["mousedown"]
            ...
            if (    self.edit != None
                ...
                if (    self.edit == len(self._points) -1
                    and self.overlap(dx, dy, x, y, r=self.btn_r)):
            ...
            if not self.editing:
                if self.edit != None:
                    ...
                    if (   self.overlap(pt.ctrl1.x, pt.ctrl1.y, x, y)
                        or self.overlap(pt.ctrl2.x, pt.ctrl2.y, x, y)):
                        self.editing = True
                    else:
                        self.edit = None
